...
class aclient(discord.AutoShardedClient):
    ...
    async def on_member_join(self, member: discord.Member) -> None:
        ...
        c.execute(
            "SELECT account_age_min FROM servers WHERE guild_id = ?", (member.guild.id,)
        )
        result = c.fetchone()
        ...
        if account_age_in_seconds(member) < account_age_min:
            try:
                await member.kick(
                    reason=f"Account age is less than {Functions.format_seconds(account_age_min)}."
                )
                await Functions.send_logging_message(
                    member=member, kind="account_too_young"
                )
                return
            except discord.Forbidden:
                return
            ...
            program_logger.debug(
                f"Account age is greater than {Functions.format_seconds(account_age_min)}."
            )
        ...
        if result is None or result[0] is None:
            return
        c.execute(
            "INSERT INTO processing_joined VALUES (?, ?, ?)",
            (
                member.guild.id,
                member.id,
                int(
    ...
    async def on_member_remove(self, member: discord.Member) -> None:
        if not self.initialized:
            return
        c.execute(
            "DELETE FROM processing_joined WHERE guild_id = ? AND user_id = ?",
            (
                member.guild.id,
                member.id,
            ),
    ...
    async def on_interaction(self, interaction: discord.Interaction) -> None:
        ...
        class WhyView(discord.ui.View):
            def __init__(self, *, timeout=None):
                ...
                self.add_item(
                    discord.ui.Button(
                        label="Secure your server",
                        url=f"https://discord.com/api/oauth2/authorize?client_id={bot.user.id}&permissions=268503046&scope=bot%20applications.commands",
                        style=discord.ButtonStyle.link,
                    )
                )

        if (
            interaction.data and interaction.data.get("component_type") == 2
        ):  # 2 is the component type for button
...
class update_stats:
                ...
                async with session.post(
                    ...
                    if resp.status != 200:
                        program_logger.error(
                            f"Failed to update top.gg: {resp.status} {resp.reason}"
                        )
            try:
    ...
    async def discordlist() -> None:
        ...
        while not shutdown:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"https://api.discordlist.gg/v0/bots/{bot.user.id}/guilds",
                    headers=headers,
                    json={"count": len(bot.guilds)},
                ) as resp:
                    if resp.status != 200:
                        program_logger.error(
                            f"Failed to update discordlist.gg: {resp.status} {resp.reason}"
                        )
            try:
                await asyncio.sleep(60 * 30)
            except asyncio.CancelledError:
        ...
        while not shutdown:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status != 200:
                        program_logger.error(
                            f"Failed to update discord.bots.gg: {resp.status} {resp.reason}"
                        )
            try:
                await asyncio.sleep(60 * 30)
            except asyncio.CancelledError:
    ...
    async def discords() -> None:
        headers = {"Authorization": discords_token, "Content-Type": "application/json"}
        while not shutdown:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"https://discords.com/bots/api/bot/{bot.user.id}",
                    headers=headers,
                    json={"server_count": len(bot.guilds)},
                ) as resp:
                    if resp.status != 200:
                        program_logger.error(
                            f"Failed to update discords.com: {resp.status} {resp.reason}"
                        )
            try:
                await asyncio.sleep(60 * 30)
            except asyncio.CancelledError:
        ...
        while not shutdown:
            async with aiohttp.ClientSession() as session:
                async with session.patch(
                    f"https://api.discord-botlist.eu/v1/update",
                    headers=headers,
                    json={"serverCount": len(bot.guilds)},
                ) as resp:
                    if resp.status != 200:
                        program_logger.error(
                            f"Failed to update discordlist.gg: {resp.status} {resp.reason}"
...
class Functions:
    ...
    async def verify(interaction: discord.Interaction) -> None:
                ...
                if self.answer.value.upper() == captcha_text:
                    try:
                        await interaction.user.add_roles(
                            interaction.guild.get_role(int(verified_role_id))
                        )
                        await Functions.send_logging_message(
                            interaction=interaction, kind="verify_success"
                        )
                        await interaction.response.edit_message(
                            content="You have successfully verified yourself.",
                            view=None,
                        )
                        c.execute(
                            "DELETE FROM processing_joined WHERE guild_id = ? AND user_id = ?",
                        ...
                        await interaction.response.edit_message(
                            content="I do not have the permission to add the verified role.",
                            view=None,
                        )
                    except discord.errors.NotFound:
                        pass
                    ...
                    await Functions.send_logging_message(
                        interaction=interaction, kind="verify_fail"
                    )
                    await interaction.response.edit_message(
                        content="The captcha text you entered is incorrect.", view=None
                    )
            ...
            async def on_timeout(self):
                if not captcha_input.verification_successful:
                    ...
                    await interaction.edit_original_response(
                        content="Captcha timed out. Request a new one.", view=None
                    )
                    if interaction.user.id in bot.captcha_timeout:
                        bot.captcha_timeout.remove(interaction.user.id)
        ...
        c.execute(
            "SELECT verify_role FROM servers WHERE guild_id = ?",
            (interaction.guild_id,),
        )
        verified_role_id = c.fetchone()
        if not verified_role_id:
            await interaction.response.send_message(
                "No verified role set. Please contact the server administrator.",
                ephemeral=True,
            )
            return
        ...
        await Functions.send_logging_message(
            interaction=interaction, kind="verify_start"
        )
    ...
    async def process_latest_joined() -> None:
        while not shutdown:
            ...
            for guild in bot.guilds:
                try:
                    c.execute(
                        "SELECT timeout, verify_role, action, ban_time FROM servers WHERE guild_id = ?",
                        (guild.id,),
                    )
                    server = c.fetchone()
                    if not server:
                    ...
                    c.execute(
                        "SELECT user_id FROM processing_joined WHERE guild_id = ? AND (join_time + ?) < ?",
                        (guild.id, timeout, current_time),
                    )
                    rows = c.fetchall()
                    if not rows:
                        continue
                    ...
                    for (user_id,) in rows:
                        ...
                        if action == "kick":
                            try:
                                await member.kick(
                                    reason="Did not successfully verify in time."
                                )
                                await Functions.send_logging_message(
                                    member=member, kind="verify_kick"
                                )
                                program_logger.debug(f"Kicked {member} from {guild}.")
                            except discord.Forbidden:
                                program_logger.debug(
                                    f"Could not kick {member} from {guild}."
                                )
                        elif action == "ban":
                            try:
                                await member.ban(
                                    reason=(
                                        f"Did not successfully verify in time. Banned for {Functions.format_seconds(ban_time)}"
                                        if ban_time
                                        else "Did not successfully verify in time."
                                    )
                                )
                                if ban_time:
                                    c.execute(
                                        "INSERT INTO temp_bans VALUES (?, ?, ?)",
                                        (guild.id, member.id, current_time + ban_time),
                                    )
                                await Functions.send_logging_message(
                                    member=member, kind="verify_ban"
                                )
                                program_logger.debug(f"Banned {member} from {guild}.")
                            except discord.Forbidden:
                                program_logger.debug(
                                    f"Could not ban {member} from {guild}."
                                )
                        c.execute(
                            "DELETE FROM processing_joined WHERE guild_id = ? AND user_id = ?",
                            (guild.id, user_id),
        ...
        while not shutdown:
            ...
            for temp_ban in temp_bans:
                ...
                try:
                    ...
                    if guild is None:
                        c.execute(
                            "DELETE FROM temp_bans WHERE guild_id = ?", (guild_id,)
                        )
                        continue
                    ...
                    try:
                        ...
                        embed = discord.Embed(
                            title="Unban",
                            description=f"User {member.mention} was unbanned.",
                            color=discord.Color.green(),
                        )
                        embed.timestamp = datetime.datetime.now(datetime.UTC)
                        program_logger.debug(
                            f"Unbanned {member.name}#{member.discriminator} ({member.id}) from {guild.name} ({guild.id})."
                        )
                        c.execute(
                            "DELETE FROM temp_bans WHERE guild_id = ? AND user_id = ?",
                            (guild_id, user_id),
                        ...
                        if log_channel:
                            try:
                                ...
                                program_logger.debug(
                                    f"Could not send unban log message in {guild.name} ({guild.id})."
                                )
                    except discord.Forbidden:
                        program_logger.debug(
                            f"Could not unban {member.name}#{member.discriminator} ({member.id}) from {guild.name} ({guild.id})."
                        )
                except Exception as e:
                    conn.commit()
    ...
    async def send_logging_message(
        ...
        c.execute(
            "SELECT log_channel, ban_time, account_age_min FROM servers WHERE guild_id = ?",
            (guild_id,),
        )
        row = c.fetchone()
        if not row:
        ...
        log_channel = (interaction.guild if interaction else member.guild).get_channel(
            log_channel_id
        )
        if not log_channel:
            return
        ...
        if kind == "verify_start":
            ...
            embed.description = (
                f"User {interaction.user.mention} requested a new captcha."
            )
            embed.color = discord.Color.blurple()
        elif kind == "verify_success":
            embed.title = "Verification successful"
            embed.description = (
                f"User {interaction.user.mention} successfully verified."
            )
            embed.color = discord.Color.green()
        elif kind == "verify_fail":
            embed.title = "Wrong captcha"
            embed.description = (
                f"User {interaction.user.mention} entered a wrong captcha."
            )
            ...
            if ban_time:
                embed.add_field(
                    name="Duration", value=f"{Functions.format_seconds(ban_time)}"
                )
        elif kind == "verify_mass_started":
            embed.title = "Mass verification started"
            embed.description = (
                f"Mass verification started by {interaction.user.mention}."
            )
            embed.color = discord.Color.blurple()
            ...
            embed.description = (
                f"User {member.mention} was verified by {interaction.user.mention}."
            )
            embed.color = discord.Color.green()
        ...
        try:
            ...
            program_logger.debug(
                f"Sent logging message in {log_channel.guild.name} ({log_channel.guild.id}) with type {kind}."
            )
...
async def self(interaction: discord.Interaction):
    ...
    class CaptchaView(discord.ui.View):
        def __init__(self, *, timeout=None):
            ...
            self.add_item(
                discord.ui.Button(
                    label="Why?", style=discord.ButtonStyle.blurple, custom_id="why"
                )
            )

    c.execute(
        "SELECT verify_channel, timeout, action, ban_time FROM servers WHERE guild_id = ?",
        (interaction.guild.id,),
    )
    data = c.fetchone()
    if not data:
        await interaction.followup.send(
            "The verification channel is not set. Please set it with `/setup`.",
            ephemeral=True,
        )
        return
    ...
    try:
        ...
        await interaction.followup.send(
            f"I don't have permission to see the verification channel (<#{verify_channel_id}>).",
            ephemeral=True,
        )
        return
    ...
    if panel_id:
        try:
            ...
            await interaction.followup.send(
                "The verification panel already exists.\nTo update it, you have to first delete the old one.",
                ephemeral=True,
            )
            return
        except (discord.NotFound, discord.Forbidden):
    ...
    try:
        ...
        await interaction.followup.send(
            f"I don't have permission to send messages in the verification channel (<#{verify_channel_id}>).",
            ephemeral=True,
        )
        return

    c.execute(
        "INSERT OR REPLACE INTO panels VALUES (?, ?)", (interaction.guild_id, panel.id)
    )
    conn.commit()
    await interaction.followup.send(
        f"The verification panel has been sent to <#{verify_channel_id}>.",
        ephemeral=True,
    )
...
async def self(
    ...
    if action == "kick" and not guild_me.guild_permissions.kick_members:
        await interaction.followup.send(
            f"I need the permission to {action} members.", ephemeral=True
        )
        return
    ...
    if not verify_channel.permissions_for(guild_me).send_messages:
        await interaction.followup.send(
            f"I need the permission to send messages in {verify_channel.mention}.",
            ephemeral=True,
        )
        return
    ...
    if guild_me.top_role <= verify_role:
        await interaction.followup.send(
            f"My highest role needs to be above {verify_role.mention}, so I can assign it.",
            ephemeral=True,
        )
        return
    ...
    if not bot_permissions.view_channel:
        await interaction.followup.send(
            f"I need the permission to see {log_channel.mention}.", ephemeral=True
        )
        return
    if not (bot_permissions.send_messages and bot_permissions.embed_links):
        await interaction.followup.send(
            f"I need the permission to send messages and embed links in {log_channel.mention}.",
            ephemeral=True,
        )
        return
    ...
    if ban_time:
        ...
        if ban_time is None:
            await interaction.followup.send(
                "Invalid ban time. Please use the following format: `1d / 1h / 1m / 1s`.\nFor example: `1d2h3m4s`",
                ephemeral=True,
            )
            return
    ...
    if account_age:
        if not guild_me.guild_permissions.kick_members:
            await interaction.followup.send(
                f"I need the permission to kick members.", ephemeral=True
            )
            return
        account_age = timeparse(account_age)
        if account_age is None:
            await interaction.followup.send(
                "Invalid account age. Please use the following format: `1d / 1h / 1m / 1s`.\nFor example: `1d2h3m4s`",
                ephemeral=True,
            )
            return
    ...
    conn.commit()
    await interaction.followup.send(
        f"Setup completed.\nYou can now run `/send_panel`, to send the panel to <#{verify_channel.id}>.",
        ephemeral=True,
    )
...
async def self(interaction: discord.Interaction):
    ...
    c.execute(
        "SELECT verify_channel, verify_role, log_channel, timeout, action, ban_time, account_age_min FROM servers WHERE guild_id = ?",
        (interaction.guild.id,),
    )
    data = c.fetchone()
    if data:
        ...
        await interaction.followup.send(
            "There are no settings for this server.\nUse `/setup` to set-up this server.",
            ephemeral=True,
        )
...
async def self(interaction: discord.Interaction):
    ...
    c.execute(
        "SELECT verify_role FROM servers WHERE guild_id = ?", (interaction.guild.id,)
    )
    data = c.fetchone()
    if not data:
        await interaction.followup.send(
    ...
    if not verify_role_id:
        await interaction.followup.send(
            "The verify role does not exist.", ephemeral=True
        )
        return
    ...
    await interaction.followup.send(
        "Verifying all users on the server. This can take a while.", ephemeral=True
    )
    await Functions.send_logging_message(
        interaction=interaction, kind="verify_mass_started"
    )
    ...
    for member in members_to_verify:
        try:
            await member.add_roles(
                verify_role, reason="Verify all users on the server."
            )
        except discord.Forbidden:
            continue
    ...
    await Functions.send_logging_message(
        ...
        kind="verify_mass_success",
        mass_amount=len(members_to_verify),
    )
    await interaction.edit_original_response(
...
@discord.app_commands.checks.cooldown(
    1, 60, key=lambda i: (i.guild_id, i.data["target_id"])
)
...
async def verify_user(interaction: discord.Interaction, member: discord.Member):
    ...
    if member.bot or verify_role in member.roles:
        await interaction.followup.send(
            f"{member.mention} is already verified or is a bot.", ephemeral=True
        )
        return
    ...
    try:
        await member.add_roles(
            verify_role,
            reason=f"{interaction.user.name} verified user via context menu.",
        )
        await interaction.followup.send(
            f"{member.mention} got verified by {interaction.user.mention}.",
            ephemeral=True,
        )
        await Functions.send_logging_message(
            interaction=interaction, kind="user_verify", member=member
        )
    except discord.Forbidden:
        await interaction.followup.send(
            "I do not have permission to add roles to this user.", ephemeral=True
        )
